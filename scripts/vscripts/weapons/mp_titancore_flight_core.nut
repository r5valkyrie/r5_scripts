...
void function PROTO_FlightCore( entity titan, float flightTime )
{
	...
	if ( titan.IsPlayer() )
	{
		...
		titan.GiveWeapon( "mp_titanweapon_flightcore_rockets", WEAPON_INVENTORY_SLOT_PRIMARY_2 )
		titan.SetActiveWeaponByName( eActiveInventorySlot.mainHand, "mp_titanweapon_flightcore_rockets" )
	...
	else
	{
		...
		if ( weapons.len() > 0 && weapons[0] )
			titan.SetActiveWeaponByName( eActiveInventorySlot.mainHand, weapons[0].GetWeaponClassName() )
	}*/
}
#endif