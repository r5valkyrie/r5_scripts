...
bool function RTKPaginationNavController_IsTargetPageLocked( rtk_behavior self, int targetPage )
{
	...
	if ( RTKDataModel_HasDataModel( pipModelPath ) )
	{
		...
		if ( pipArray == null )
			return false
		rtk_struct pipStruct = RTKArray_GetStruct( pipArray, targetPage )
...
void function RTKPaginationNavController_UpdateNavArrowsState( rtk_behavior self )
{
	...
	if ( p.pageIndex == 0 )
	{
			prevButton.PropSetBool( "interactive", false )
	}
	else if ( p.pageIndex == p.pageCount )
	{
			nextButton.PropSetBool( "interactive", false )
	}
	else
	{
			int nextPage = ClampInt( p.pageIndex + 1, 0, p.pageCount )
			int prevPage = ClampInt( p.pageIndex - 1, 0, p.pageCount )

			if ( RTKPaginationNavController_IsTargetPageLocked( self, nextPage ) )
			{
				nextButton.PropSetBool( "interactive", false )
				return
			}
			if ( RTKPaginationNavController_IsTargetPageLocked( self, prevPage ) )
			{
				prevButton.PropSetBool( "interactive", false )
				return
			}

			nextButton.PropSetBool( "interactive", true )
			prevButton.PropSetBool( "interactive", true )
	}
}