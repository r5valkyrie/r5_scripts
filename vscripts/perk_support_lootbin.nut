...
int function SupportBin_GetPlayerSurvivalNeed( entity player )
{
	...
	if( survivalItemStatus != SurvivalStatusType.SURIVIAL_LOOT_HAS_HS && ( DeathField_PointDistanceFromFrontierForIndex( player.GetOrigin(), player.DeathFieldIndex() ) <= 0.0 ) )
	{
...
void function Thread_SupportBin_TrackPlayerRinglocation( entity player )
{
	...
	while ( true )
	{
		...
		if ( DeathField_PointDistanceFromFrontierForIndex( player.GetOrigin(), player.DeathFieldIndex() ) >= 0.0 )
		{
