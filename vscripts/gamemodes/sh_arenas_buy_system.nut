...
void function Arenas_OnLootDatatablesLoaded()
{
	...
	printt( "[Arenas BuySystem] CLIENT - store items loaded:", file.storeDataArray.len() )
	Arenas_InitWeaponData()
	printt( "[Arenas BuySystem] CLIENT - weapon upgrades loaded:", file.weaponUpgrades.len() )
	Arenas_InitWeaponTabs()
	printt( "[Arenas BuySystem] CLIENT - weapon tabs loaded:", file.weaponTabs.len() )
}
#endif
...
void function Arenas_InitStoreData()
{
	...
	printt( "[Arenas BuySystem] InitStoreData - disabled weapons:", disabledWeapons.len() )

	table<string, LootData> allLootData = SURVIVAL_Loot_GetLootDataTable()
	printt( "[Arenas BuySystem] InitStoreData - loot data entries:", allLootData.len() )
	...
	for( int i = 0; i < numRows; ++i )
	{
		...
		printt( "[Arenas BuySystem] InitStoreData - equipment item", i, "ref:", item.ref, "lootOrPerkRef:", item.lootOrPerkRef, "category:", item.category, "cost:", item.cost, "storeIndex:", item.index )
...
void function Arenas_InitWeaponData()
{
	...
	for( int i = 0; i < numRows; ++i )
	{
		...
		if( weaponRef != "" && weaponRef.find( "!!data excluded!!" ) == -1 )
		{
			...
			if ( !(weaponRef in file.weaponUpgrades) )
				file.weaponUpgrades[ weaponRef ] <- split( GetDataTableString( dataTable, i, col_attachments ), " " )
			else
...
void function Arenas_InitWeaponTabs()
{
	...
	for( int i = 0; i < numRows; ++i )
	{
		...
		if( tabRef != "" && tabRef.find( "!!data excluded!!" ) == -1 )
		{
		...
		else if( currentTab.name != "" )
		{
			...
			if( weaponRef.find( "!!data excluded!!" ) != -1 )
				continue
...
#endif // CLIENT - DataTable init functions
...
void function OnCurrentCashChanged( entity player )
{
	...
	printt( "[Arenas BuySystem] OnCurrentCashChanged - cash:", cash, "weapons:", SURVIVAL_GetPrimaryWeapons( player ).len() )
	if( cash <= 0 && SURVIVAL_GetPrimaryWeapons( player ).len() >= 2 )
...
int function Arenas_GetShopButtonIndex( entity player, string entVar )
{
	...
	{
		branch = "player_weapon"
		result = GetPlayerWeapon( player, entVar )
	}
	...
	{
		branch = "weapon_grid"
		result = GetWeaponButtonIdx( player, entVar )
	}
	else
	{
...
ArenasStoreItem ornull function GetWeaponDataForButtonIndex( entity player, int searchIdx )
{
	...
	// During buy phase, player has no physical weapons; selections track purchases.
	table<string, string> selectedHighestByBase
	foreach( selectedIdx in file.selectedItems )
	{
		if( selectedIdx >= file.storeDataArray.len() )
			continue
		ArenasStoreItem selectedItem = file.storeDataArray[ selectedIdx ]
		if( !SURVIVAL_Loot_IsRefValid( selectedItem.ref ) )
			continue
...
void function _Arenas_BindWeaponButton( entity player, var button, var rui, int idx, string entVar )
{
	...
	if ( player != null )
	{
		if( entVar.find( "weapon" ) != -1 )
		{
			...
			// Also check buy phase selections (weapons purchased but not yet granted)
			if( !owned && IsStoreIndexSelected( player, item.index ) )
				owned = true
...
void function SetPreferredOptic( string itemRef )
{
	...
	GetLocalClientPlayer().ClientCommand( "Arenas_SetOptic " + weaponData.index + " " + opticIndex )
}
...
void function UICallback_Arenas_BindBuyButton( var button )
{
	...
	string entVar = Hud_GetScriptID( button )
	printt( "[Arenas BuySystem] BindBuyButton - scriptID:", entVar, "storeDataArray.len:", file.storeDataArray.len() )

	int idx = Arenas_GetShopButtonIndex( player, entVar )
	printt( "[Arenas BuySystem] BindBuyButton - idx:", idx, "for:", entVar )
...
void function _Arenas_BindBuyButtonV2( entity player, var button, var rui, int idx, string entVar )
{
	...
	if ( item.ref == "arenas_full_ultimate" )
	{
		...
		RuiSetBool( rui, "hasPreReq", hasPreReq && ( player.GetPlayerNetInt( "ultimateCooldown" ) <= 0 || owned ) )
	}
	...
	if ( button != null )
	{
		...
		//
		if( !locked && !wasOwned && !ownsMaxItems )
...
void function UICallback_Arenas_OnBuyButtonClick( var button )
{
	...
	int actualCost = GetArenasStoreItemCost( player, item.ref )
	GetLocalClientPlayer().ClientCommand( "Arenas_Select " + idx + " " + actualCost + " " + item.lootOrPerkRef )
...
void function UICallback_Arenas_OnBuyButtonRightClick( var button )
{
	...
	if( IsAvailableWeapon( player, item.lootOrPerkRef ) )
	{
		...
		if ( GetNextUpgradedWeaponRef( item.lootOrPerkRef ) == item.lootOrPerkRef )
		{
			...
			// Also check buy phase selections (weapons purchased but not yet granted)
			if ( !owned && IsStoreIndexSelected( player, idx ) )
				owned = true
	...
	ArenasStoreItem removedItem = GetStoreDataByIndex( indexToRemove )
	file.isProcessingClickEvent = true
	EmitSoundOnEntity( GetLocalClientPlayer(), SOUND_SELL )
	int actualCost = GetArenasStoreItemCost( player, removedItem.ref )
	GetLocalClientPlayer().ClientCommand( "Arenas_Unselect " + indexToRemove + " " + actualCost + " " + removedItem.lootOrPerkRef )
...
void function UICallback_Arenas_OnOpticSlotButtonClick( var button )
{
	...
	GetLocalClientPlayer().ClientCommand( "Arenas_SetOptic " + currentWeaponData.index + " " + opticLootIndex )
...
bool function PlayerHasRequiredWeapon( entity player, string ref )
{
	...
	if( data.lootType == eLootType.MAINWEAPON )
	{
		if ( WeaponLootRefIsLockedSet( lootRef ) )
		{
			...
			if( prevRef in file.storeData )
			{
				int prevIdx = file.storeData[ prevRef ].index
				if( IsStoreIndexSelected( player, prevIdx ) )
					return true
			}
...
string function GetOffhandWeaponRef( entity player, string ref )
{
	if ( ref == "tactical_upgrade" || ref == "arenas_full_ultimate" )
	{
		...
		if ( ref == "tactical_upgrade" )
			...
			abilityFlavor = CharacterClass_GetUltimateAbility( character )
		return CharacterAbility_GetWeaponClassname( abilityFlavor )
	}
...
bool function IsPlayerMaxedOut( entity player, string ref )
{
	...
	{
		// Count how many times this ref has been purchased this round
		int purchaseCount = 0
		foreach ( selectedIdx in file.selectedItems )
		{
			ArenasStoreItem storeItem = GetStoreDataByIndex( selectedIdx )
			if ( storeItem.ref == ref )
				purchaseCount++
		}

		// Check against store maxCount
		if ( ref in file.storeData && file.storeData[ref].maxCount > 0 )
		{
			if ( purchaseCount >= file.storeData[ref].maxCount )
				return true
		}

		// Secondary validation from weapon state
		if ( ref == "tactical_upgrade" )
		{
			entity ability = player.GetOffhandWeapon( OFFHAND_TACTICAL )
			if ( IsValid( ability ) )
				return ability.GetWeaponPrimaryAmmoCount( AMMOSOURCE_STOCKPILE ) == ability.GetWeaponPrimaryAmmoCountMax( AMMOSOURCE_STOCKPILE )
		}
		else if ( ref == "arenas_full_ultimate" )
		{
			entity ability = player.GetOffhandWeapon( OFFHAND_ULTIMATE )
			if ( IsValid( ability ) )
...
{
	printt( "[Arenas BuySystem] OpenBuyMenu called, endTime:", endTime, "storeItems:", file.storeDataArray.len(), "tabs:", file.weaponTabs.len() )
...
void function _OpenBuyMenu( float endTime, bool playIntroTransition )
{
	if ( Time() >= endTime )
	{
		printt( "[Arenas BuySystem] _OpenBuyMenu - time already past endTime, skipping" )
		return
	}

	printt( "[Arenas BuySystem] _OpenBuyMenu - opening UI, playIntro:", playIntroTransition )
