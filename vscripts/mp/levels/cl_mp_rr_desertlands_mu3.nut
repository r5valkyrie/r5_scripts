...
void function LavaSiphon_SpawnFX_Thread()
{
	...
	while( true )
	{
		...
		// Using ambient generics so audio fades at a predefined distance
		entity sfx = CreateClientSideAmbientGeneric( refEnt.GetOrigin(), data.sfxString, 0 )

		// Need to clean up fx, otherwise lingers in world
...
{
	// We don't want to spawn an effect on the same entity if it exists, so make sure we get a different one
	if ( IsValid( refEnt ) )
	{
