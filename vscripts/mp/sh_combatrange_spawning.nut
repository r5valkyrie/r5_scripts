		...
		if( radiusSpawn )
		{
			...
			vector randomPoint = ptsInRadius.getrandom()
				...
				if( file.dev_spawnPrintsOn )
				{
					...
					DebugDrawCircle( destination, <0, 0, 0>, spawnRadius, COLOR_WHITE, true, circleTime )
					DebugDrawSphere( destination, debugRadius, COLOR_RED, true, circleTime )
					DebugDrawSphere( randomPoint, debugRadius, COLOR_BLUE, true, circleTime )
				}
			#endif
...
entity function DoDummieSpawn( int realm, vector destination, vector targetFacing, int spawnSoundType = eCRng_DummieSpawningTypes.CULL_RESPAWN, bool debugDraws = false )
{
	...
	else
	{
		...
		{
			DebugDrawCube( newCRDummie.GetOrigin(), 64, COLOR_RED, true, 10 )
		}
	#endif
...
void function DEV_CombatRange_Show_SpawnPoints( float showTime = 10 )
{
	...
	{
		if( spawnPoint.IsOccupied( refPlayer ) )
		{
			DebugDrawSphere( spawnPoint.GetOrigin(), 50, COLOR_RED, true, showTime )
		}
		else
		{
			if( IsFacing( players[0], spawnPoint, COMBATRANGE_SPAWNPOINT_FACING_THRESHOLD, false ) )
			{
				...
				{
					DebugDrawCube( spawnPoint.GetOrigin(), 25, COLOR_GREEN, true, showTime )
				}
				else
				{
					DebugDrawCube( spawnPoint.GetOrigin(), 25, COLOR_ORANGE, true, showTime )
				}
			}
			else
			{
...
{
	DebugDrawCube( origin, size, color, throughGeo, showTime )
}
...
void function DEV_CountDummies( entity playerParm )
{
	...
	foreach( dummie in file.dummiesSpawnedInRealm[ realm ] )
	{
		DebugDrawSphere( dummie.GetOrigin(), 48, COLOR_GREEN, true, 20 )
	}
	...
	foreach( dummie in allDummiesByName )
	{
		if( dummie.GetRealms()[ 0 ] == realm )
		{
			...
			DebugDrawCube( dummie.GetOrigin(), 32, COLOR_AQUA, true, 20 )
			...
			{
				DebugDrawSphere( dummie.GetOrigin(), 64, COLOR_RED, true, 20 )
			}
		}
	}
