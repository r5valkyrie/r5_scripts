...
global function ClientToUI_ShipVaultConfirmDialog_Open
...
{
	if ( !fileVM.levelInitDone )
	{
		NEW_SurvivalGroundList_LevelInit()
		fileVM.levelInitDone = true
	}

	if ( "DeathBoxListPanel_VMInit" in getroottable() )
...
#if UI
void function NEW_OpenSurvivalGroundListMenu( bool playerIsTitan = false )
{
	CloseAllMenus()
...
#if CLIENT
bool function IsBlackMarketDeathBox( entity deathBox )
{
	if ( !IsValid( deathBox ) )
		return false
...
{
	if ( !IsValid( player ) || !IsValid( deathBox ) )
		return []

	bool shouldUseNearby = (Survival_GetGroundListBehavior() == eGroundListBehavior.NEARBY)
	if ( !shouldUseNearby )
	{
	...
	if ( shouldUseNearby )
		return GetSurvivalLootNearbyPlayer( player, SURVIVAL_GROUNDLIST_NEARBY_RADIUS, false, false )
...
{
	if ( !fileLevel.levelInitDone )
	{
		NEW_SurvivalGroundList_LevelInit()
		fileLevel.levelInitDone = true
	}
	...
	bool isBlackMarket = IsBlackMarketDeathBox( deathBox )
	if ( isBlackMarket )
	{
		entity localPlayer = GetLocalClientPlayer()
		if ( IsValid( localPlayer ) )
			EmitSoundOnEntity( localPlayer, "Loba_Ultimate_BlackMarket_Open" )
	...
	array<entity> openLootEnts = []
	openLootEnts = GetLootEntsForCurrentGroundList( GetLocalClientPlayer(), deathBox )
	Warning( "NEW_GROUNDLIST_OPEN: class=%s eeh=%d loot=%d", deathBox.GetNetworkedClassName(), fileLevel.currentDeathBoxEEH, openLootEnts.len() )

	NEW_SurvivalGroundListUpdateParams openParams
	openParams.player = GetLocalClientPlayer()
	openParams.currentLootEnts = openLootEnts
	openParams.isBlackMarket = isBlackMarket
	openParams.blackMarketUseCount = -1
	NEW_UpdateSurvivalGroundList( openParams )
	thread NEW_LiveUpdateThread( openParams.player, deathBox )
	...
	else
	{
		...
		if ( customOwnerName != "" )
		{
			...
			if ( IsValidLoadoutSlotContentsIndex( Loadout_Character(), characterIdx ) )
			{
...
void function UIToClient_NEW_SurvivalGroundListClosed()
{
	...
	entity deathBox = Survival_GetDeathBox()
	if ( IsBlackMarketDeathBox( deathBox ) )
	{
		entity localPlayer = GetLocalClientPlayer()
		if ( IsValid( localPlayer ) )
			EmitSoundOnEntity( localPlayer, "Loba_Ultimate_BlackMarket_Close" )
	}
...
void function NEW_UpdateSurvivalGroundList( NEW_SurvivalGroundListUpdateParams params )
{
	...
	array<entity> currentLootEnts = params.currentLootEnts
	if ( currentLootEnts.len() == 0 )
	{
		currentLootEnts = GetLootEntsForCurrentGroundList( params.player, deathBox )
	}
	...
	table<string, void> entriesToUpdateSet = {}

	//
	foreach ( entity lootEnt in currentLootEnts )
	{
		...
		if ( lootEnt in fileLevel.deathBoxEntryDataByLootEnt )
		{
			...
			if ( lootEnt.GetClipCount() != lootEnt.e.deathBoxMenu_lastSeenClipCount )
			{
				...
				entriesToUpdateSet[entryData.key] <- IN_SET
			}
			continue
		}
		...
		for ( ; otherLootEntIdx < entryData.lootEnts.len(); otherLootEntIdx++ )
		{
			...
			bool lootEntIsCausticTTItem      = (IsValid( lootEntCausticTT ) && AreCausticTTCanistersClosed(lootEntCausticTT))
			entity otherLootEntCausticTT = GetCausticTTCanisterFrameForLoot( otherLootEnt )
			bool otherLootEntIsCausticTTItem  = (IsValid( otherLootEntCausticTT ) && AreCausticTTCanistersClosed(lootEntCausticTT))
	...
	bool deathBoxBlocked = false
	{
		...
		if ( IsBlackMarketDeathBox( deathBox ) && GetBlackMarketUseCount( deathBox, GetLocalClientPlayer() ) >= GetBlackMarketUseLimit( deathBox, GetLocalClientPlayer() ) )
	...
	foreach ( string entryDataKey, void _ in entriesToUpdateSet )
	{
		if ( !(entryDataKey in fileLevel.deathBoxEntryDataByKey) )
			continue
		DeathBoxEntryData entryData = fileLevel.deathBoxEntryDataByKey[entryDataKey]
		...
		foreach ( entity lootEnt in entryData.lootEnts )
		{
			...
			if ( isMainWeapon )
				...
				entryData.totalCount += maxint( 0, lootEnt.GetClipCount() )
			else
				entryData.totalCount += maxint( 1, lootEnt.GetClipCount() )
		}
...
void function UpdateItem( DeathBoxListPanelItem item )
{
	...
	if ( IsValid( bestLootEnt ) && entryData.isUsable )
	{
		if ( isBlackMarket )
		{
			...
			if ( IsValid( causticTTPanel ) && AreCausticTTCanistersClosed(causticTTPanel) )
...
#if CLIENT
void function NEW_LiveUpdateThread( entity player, entity deathBox )
{
	EndSignal( fileLevel.signalDummy, "SurvivalGroundList_Closed" )

	while ( NEW_IsGroundListMenuOpen() )
	{
		if ( !IsValid( player ) || !IsValid( deathBox ) )
			break

		array<entity> liveLootEnts = []
		bool isBlackMarket = IsBlackMarketDeathBox( deathBox )
		liveLootEnts = GetLootEntsForCurrentGroundList( player, deathBox )

		NEW_SurvivalGroundListUpdateParams liveParams
		liveParams.player = player
		liveParams.currentLootEnts = liveLootEnts
		liveParams.isBlackMarket = isBlackMarket
		liveParams.blackMarketUseCount = -1
		NEW_UpdateSurvivalGroundList( liveParams )
...
void function PerformItemAction( DeathBoxListPanelItem item, bool isAltAction, bool fromExtendedUse, bool isFromQuickSwap, int ornull backpackSwapSlot )
{
	...
	else
	{
		...
		if ( isBlackMarket && entryData.isVaultItem )
		{
			...
			if ( vaultData.panelScriptName == "LootVaultPanel" )
				RunUIScript( "ClientToUI_LootVaultConfirmDialog_Open", deathBox.GetOwner() == player )

			else if ( vaultData.panelScriptName == "ShipVaultPanel" )
				RunUIScript( "ClientToUI_ShipVaultConfirmDialog_Open", deathBox.GetOwner() == player )
	...
	if ( showUseHighlight )
	{
		...
		entity localPlayer = GetLocalClientPlayer()
		if ( IsValid( localPlayer ) )
			EmitSoundOnEntity( localPlayer, "ui_menu_accept" )
	}
...
void function ClientToUI_ShipVaultConfirmDialog_Open( bool isBlackMarketOwner )
{
	ConfirmDialogData data
	data.headerText = Localize( "#LOBA_ULT_BLACK_MARKET_BRIDGE_VAULT_CONFIRM_HEADER" )

	if ( isBlackMarketOwner )
	{
		data.messageText = Localize( "#LOBA_ULT_BLACK_MARKET_BRIDGE_VAULT_CONFIRM_BODY" )
	...
	{
		data.messageText = Localize( "#LOBA_ULT_BLACK_MARKET_BRIDGE_VAULT_DENY_BODY" )
		OpenOKDialogFromData( data )
	}
}
...
void function SendItemActionCommand( DeathBoxEntryData entryData, int count, entity preferredLootEnt, int actionType, bool isAltAction, int ornull backpackSwapSlot )
{
	...
	int deathBoxEEH = fileLevel.currentDeathBoxEEH
	string boxString = (deathBoxEEH != EncodedEHandle_null ? (" " + deathBoxEEH) : "")

	if ( backpackSwapSlot != null && IsValid( preferredLootEnt ) )
	{
		localPlayer.ClientCommand( "SwapSurvivalItem " + expect int( backpackSwapSlot ) + " " + preferredLootEnt.GetEncodedEHandle() + boxString )
	}
	else if ( actionType == eLootAction.PICKUP_ALL && IsValid( preferredLootEnt ) )
	{
		localPlayer.ClientCommand( "PickupAllSurvivalItem " + preferredLootEnt.GetSurvivalInt() )
		localPlayer.ClientCommand( "PickupSurvivalItem " + preferredLootEnt.GetEncodedEHandle() + " " + pickupFlags + boxString )
	}
	else if ( IsValid( preferredLootEnt ) )
	{
		localPlayer.ClientCommand( "PickupSurvivalItem " + preferredLootEnt.GetEncodedEHandle() + " " + pickupFlags + boxString )
	}
...
void function PingItem( DeathBoxListPanelItem item )
{
	...
	foreach ( entity lootEnt in entryData.lootEnts )
	{
		if ( IsValid( lootEnt ) && IsValid( Waypoint_GetWaypointForLootItemPingedBy( lootEnt, localPlayer ) ) )
		{
...
void function UIToClient_NEW_SurvivalGroundList_OnQuickSwapItemClick( var button, int backpackSwapSlot, bool isRightClick )
{
	...
	{
		int deathBoxEEH = fileLevel.currentDeathBoxEEH
		string boxString = (deathBoxEEH > -1 ? (" " + deathBoxEEH) : "")
		localPlayer.ClientCommand( "Sur_DropBackpackItem " + dropLootFlav.ref + " " + inventoryEntry.count + boxString )
		RefreshQuickSwapIfOpen()
	}
	else
	{
...
void function UIToClient_NEW_SurvivalGroundList_OnMouseLeftPressed()
{
	...
	if ( !IsCursorInElementBounds( fileLevel.currentQuickSwapItemButton )
	&& !IsCursorInElementBounds( fileLevel.quickSwapBacker )
	&& !IsCursorInElementBounds( fileLevel.quickSwapGrid )
	&& !IsCursorInElementBounds( fileLevel.quickSwapHeader )
	&& !IsCursorInElementBounds( fileLevel.inventorySwapIcon ) )
	{
		CloseQuickSwapIfOpen()
	}
}
