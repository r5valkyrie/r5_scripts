...
void function FlyerHovers( entity player, HoverSounds soundInfo, float flightTime = 3.0, float horizVel = 200.0 )
{
		...
		function() : ( activeFX, player, soundInfo )
		{
			if ( IsValid( player ) )
			{
				...
				if ( player.IsPlayer() )
				{
					...
					player.kv.gravity = player.GetPlayerSettingFloat( "gravityScale" )
	...
	for ( ;; )
	{
		...
		vel.z = height
		vel = LimitVelocityHorizontalr2( vel, horizVel + 50 )
...
vector function LimitVelocityHorizontalr2( vector vel, float speed )
{
