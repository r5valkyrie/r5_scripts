...
function FireWeaponPlayerAndNPC( WeaponPrimaryAttackParams attackParams, bool playerFired, entity weapon )
{
	...
	if ( shouldCreateProjectile )
	{
		...
		for ( int index = 0; index < numProjectiles; index++ )
		{
			...
			fireBoltParams.speed = projectileSpeed
			fireBoltParams.scriptTouchDamageType = damageTypes.largeCaliber | DF_SHOTGUN
			fireBoltParams.scriptExplosionDamageType = damageTypes.largeCaliber | DF_SHOTGUN
