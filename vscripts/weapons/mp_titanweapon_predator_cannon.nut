...
var function OnWeaponPrimaryAttack_titanweapon_predator_cannon( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( hasLongRangePowerShot || hasCloseRangePowerShot )
	{
		...
		if ( hasCloseRangePowerShot )
		{
			...
			FireHitscanShotgunBlast( weapon, attackParams.pos, attackParams.dir, 16, damageType, 1.0, 10.0 )
		...
		else
		{
			...
			bolt = weapon.FireWeaponBolt( fireBoltParams )
			#endif
			if ( bolt )
			{
				bolt.kv.gravity = -0.1
...
bool function IsPredatorCannonActive( entity owner, bool reqZoom = true )
{
	if ( !owner.IsNPC() )
	{
		...
		if ( owner.GetViewModelEntity().GetModelName() != $"mdl/weapons/titan_predator/atpov_titan_predator.rmdl" )
			return false
	...
	{
		return owner.GetActiveWeapon(eActiveInventorySlot.mainHand).GetWeaponClassName() == "mp_titanweapon_predator_cannon"
	}
